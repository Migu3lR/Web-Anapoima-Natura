...
(function ($, undefined) {
	$(function () {
		...
		function runPriceAgent($frm) {
			$.post("bookAdmin.php?controller=pjAdminBookings&action=pjActionGetPriceAgent", $frm.serialize()).done(function (data) {
				if (data.status === "OK") {
					var x, $parent;
					for (x in data.data) {
		...
		function getAvailability() {
			$.get("bookAdmin.php?controller=pjAdminBookings&action=pjActionGetAvailability", {
				year: arguments[0],
				month: arguments[1],
				day: arguments[2],
		...
		$("#content").on("click", ".btnConfirmation", function () {
			...
			$grid_avail.datagrid("load", "bookAdmin.php?controller=pjAdminBookings&action=pjActionGetBooking", "id", "ASC", content.page, content.rowCount);
		...
		function getCalendar(mode) {
			...
			$.get("bookAdmin.php?controller=pjAdminBookings&action=pjActionGetCalendar", opts).done(function (data) {
		...
		function onChange() {
			...
			$.get("bookAdmin.php?controller=pjAdminBookings&action=pjActionGetRoomNumbers", {
				room_id: $option.val(),
		...
		function recalcPrice() {
			$.post("bookAdmin.php?controller=pjAdminBookings&action=pjActionRecalcPrice", $(this).closest("form").serialize()).done(function (response) {
		...
		if ($dialogRoomAdd.length > 0 && dialog) {
			$dialogRoomAdd.dialog({
				...
				open: function () {
					...
					$.get("bookAdmin.php?controller=pjAdminBookings&action=pjActionAddBookingRoom", {
				...
				buttons: {
					'Save': function () {
						if (validator.form()) {
							$.post("bookAdmin.php?controller=pjAdminBookings&action=pjActionAddBookingRoom", $dialogRoomAdd.find("form").serialize()).done(function (data) {
		...
		if ($dialogRoomEdit.length > 0 && dialog) {
			$dialogRoomEdit.dialog({
				...
				open: function () {
					$dialogRoomEdit.html("");
					$.get("bookAdmin.php?controller=pjAdminBookings&action=pjActionUpdateBookingRoom", {
				...
				buttons: {
					'Update': function () {
						if (validator.form()) {
							$.post("bookAdmin.php?controller=pjAdminBookings&action=pjActionUpdateBookingRoom", $dialogRoomEdit.find("form").serialize()).done(function (data) {
								if (data.status !== undefined && data.status === 'OK') {
									getBookingRooms.call(null, booking_id, hash);
									if ($frmUpdateBooking.length > 0) {
										runPriceAgent.call(null, $frmUpdateBooking);
		...
		if ($dialogRoomDelete.length > 0 && dialog) {
			$dialogRoomDelete.dialog({
				...
				buttons: {
					'Delete': function () {
						$.post("bookAdmin.php?controller=pjAdminBookings&action=pjActionDeleteBookingRoom", {
		...
		if ($dialogConfirmation.length > 0 && dialog) {
			$dialogConfirmation.dialog({
				...
				open: function () {
					...
					$.get("bookAdmin.php?controller=pjAdminBookings&action=pjActionConfirmation", {
						"booking_id": $frmUpdateBooking.find("input[name='id']").val()
					}).done(function (data) {
				...
				buttons: {
					'Send': function () {
						if (validator.form()) {
							$.post("bookAdmin.php?controller=pjAdminBookings&action=pjActionConfirmation", $dialogConfirmation.find("form").serialize()).done(function (data) {
		...
		function formatBid (str, obj) {
			return ['<a href="bookAdmin.php?controller=pjAdminBookings&action=pjActionUpdate&id=', obj.id, '">', obj.uuid, '</a><br>', obj.created].join("");
		}
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "bookAdmin.php?controller=pjAdminBookings&action=pjActionUpdate&id={:id}"},
				          {type: "delete", url: "bookAdmin.php?controller=pjAdminBookings&action=pjActionDeleteBooking&id={:id}"}
				          ],
				columns: [{text: myLabel.id, type: "text", sortable: true, editable: false, width: 110, renderer: formatBid},
				          ...
				          {text: myLabel.client, type: "text", sortable: true, editable: true, width: 225, editableWidth: 200},
				...
				dataUrl: "bookAdmin.php?controller=pjAdminBookings&action=pjActionGetBooking",
				dataType: "json",
				fields: ['id', 'date_from', 'c_email', 'id', 'status'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "bookAdmin.php?controller=pjAdminBookings&action=pjActionDeleteBookingBulk", render: true, confirmation: myLabel.delete_confirmation},
					   {text: myLabel.exported, url: "bookAdmin.php?controller=pjAdminBookings&action=pjActionExportBooking", ajax: false}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "bookAdmin.php?controller=pjAdminBookings&action=pjActionSaveBooking&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				},
			...
			if (m !== null) {
				...
				$grid.datagrid("load", "bookAdmin.php?controller=pjAdminBookings&action=pjActionGetBooking", "created", "DESC", content.page, content.rowCount);
			}
		}
		...
		if ($("#grid_avail").length > 0 && datagrid) {
			...
			var $grid_avail = $("#grid_avail").datagrid({
				...
				dataUrl: "bookAdmin.php?controller=pjAdminBookings&action=pjActionGetBooking&uuid=-1",
				dataType: "json",
		...
		function formatId (str) {
			return ['<a href="bookAdmin.php?controller=pjInvoice&action=pjActionUpdate&id=', str, '">#', str, '</a>'].join("");
		}
		...
		if ($("#grid_invoices").length > 0 && datagrid) {
			var $grid_invoices = $("#grid_invoices").datagrid({
				buttons: [{type: "edit", url: "bookAdmin.php?controller=pjInvoice&action=pjActionUpdate&id={:id}", title: "Edit"},
				          {type: "delete", url: "bookAdmin.php?controller=pjInvoice&action=pjActionDelete&id={:id}", title: "Delete"}],
				columns: [
				...
				dataUrl: "bookAdmin.php?controller=pjInvoice&action=pjActionGetInvoices&q=" + $frmUpdateBooking.find("input[name='uuid']").val(),
				dataType: "json",
				fields: ['id', 'order_id', 'issue_date', 'due_date', 'created', 'status', 'total'],
				paginator: {
					actions: [
					   {text: myLabel.delete_title, url: "bookAdmin.php?controller=pjInvoice&action=pjActionDeleteBulk", render: true, confirmation: myLabel.delete_body}
					],
					gotoPage: true,
					paginate: true,
					total: true,
		...
		function getBookingRooms(booking_id, hash){
			$.get("bookAdmin.php?controller=pjAdminBookings&action=pjActionGetBookingRooms", {
				"booking_id": booking_id,
				"hash": hash
			}).done(function (data) {
				$("#boxRooms").html(data);				
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("option", "cache", cache);
			$grid.datagrid("load", "bookAdmin.php?controller=pjAdminBookings&action=pjActionGetBooking", "id", "ASC", content.page, content.rowCount);
			return false;
			...
			$.extend(cache, obj);
			$grid.datagrid("option", "cache", cache);
			$grid.datagrid("load", "bookAdmin.php?controller=pjAdminBookings&action=pjActionGetBooking", "id", "DESC", content.page, content.rowCount);
			return false;
