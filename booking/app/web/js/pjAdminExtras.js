	...
	$(function () {
		...
		if ($("#grid").length > 0 && datagrid) {
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "bookAdmin.php?controller=pjAdminExtras&action=pjActionUpdate&id={:id}"},
				          {type: "delete", url: "bookAdmin.php?controller=pjAdminExtras&action=pjActionDeleteExtra&id={:id}"}
				          ],
				...
				dataUrl: "bookAdmin.php?controller=pjAdminExtras&action=pjActionGetExtra",
				dataType: "json",
				fields: ['name', 'per', 'price', 'status'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "bookAdmin.php?controller=pjAdminExtras&action=pjActionDeleteExtraBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "bookAdmin.php?controller=pjAdminExtras&action=pjActionSaveExtra&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
