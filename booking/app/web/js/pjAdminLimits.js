	...
	$(function () {
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				buttons: [{type: "delete", url: "bookAdmin.php?controller=pjAdminLimits&action=pjActionDeleteLimit&id={:id}", align: "center"}],
				columns: [
				...
				dataUrl: "bookAdmin.php?controller=pjAdminLimits&action=pjActionGetLimits",
				dataType: "json",
				fields: ['room_id', 'date_from', 'date_to', 'start_on', 'min_nights', 'max_nights'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "bookAdmin.php?controller=pjAdminLimits&action=pjActionDeleteLimitBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "bookAdmin.php?controller=pjAdminLimits&action=pjActionSaveLimit&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		if ($dialogAddLimit.length > 0 && dialog) {
			$dialogAddLimit.dialog({
				...
				open: function () {
					$dialogAddLimit.html("");
					$.get("bookAdmin.php?controller=pjAdminLimits&action=pjActionAddLimit").done(function (data) {
						...
						$dialogAddLimit.find("form").validate({
							...
							submitHandler: function (form) {
								$.post("bookAdmin.php?controller=pjAdminLimits&action=pjActionAddLimit", $dialogAddLimit.find("form").serialize()).done(function (data) {
									$(".empty-page").remove();
									$("#grid").parent().show();
									var content = $grid.datagrid("option", "content");
									$grid.datagrid("load", "bookAdmin.php?controller=pjAdminLimits&action=pjActionGetLimits", "date_from", "ASC", content.page, content.rowCount);
								}).always(function () {
