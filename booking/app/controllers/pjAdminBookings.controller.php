...
class pjAdminBookings extends pjAdmin
{
	private function _calendar($selected_date, $room_id=null, $_week_start_date=null)
	{
		...
		foreach($br_arr as $rid => $br)
		{
			$tmp_arr = array();
			for($i = $tmp_week_start_date; $i <= $tmp_week_end_date; $i=$i+86400)
			{
				$_date = date('Y-m-d', $i);
				foreach($br as $_bb)
				{
					if($_bb['date_from'] <= $_date && $_date <= $_bb['date_to'])
					{
						if(empty($tmp_arr))
						{
							$tmp_arr[] = $_bb;
							...
							$exist = false;
							foreach($tmp_arr as $check)
							{
								if($_bb['date_from'] == $check['date_from'])
								{
									$exist = true;
								}
							}
	...
	public function pjActionAddBookingRoom()
	{
		...
		if ($this->isXHR() && $this->isLoged())
		{
			...
			{
				$this->set('room_number_arr', $this->getRoomNumbers($_GET['room_id'], $_GET['date_from'], $_GET['date_to']));
			}
		}
	}
	...
	public function pjActionDeleteBookingBulk()
	{
		...
		if ($this->isXHR() && $this->isLoged())
		{
			...
			{
				pjBookingModel::factory()->whereIn('id', $_POST['record'])->limit(count($_POST['record']))->eraseAll();
		...
		if ($this->isXHR() && $this->isLoged())
		{
			...
			if (!empty($ids))
			{
				...
				foreach ($data as $k => $v)
				{
					...
					if(!empty($v['c_email']))
					{
						$client_arr[] = '<span class="link-color">' . $v['c_email'] . '</span>';
					}
	...
	private function getRoomNumbers($room_id, $date_from, $date_to)
	{
		...
		$rr_arr = pjRestrictionRoomModel::factory()
			...
			->where('t2.date_from <=', $date_to)
			->where('t2.date_to >=', $date_from)
			->findAll()
		...
		$arr = $pjRoomNumberModel->where('t1.room_id', $room_id)->orderBy('t1.id ASC')->findAll()->getData();
		...
		$room_number_arr = array();
		if (!empty($booking_rooms))
		{
			foreach ($arr as $k => $v)
			{
				if (!in_array($v['id'], $booking_rooms))
				{
					$room_number_arr[] = $v;
				}
			}
	...
	public function pjActionGetRoomNumbers()
	{
		...
		if ($this->isXHR() && $this->isLoged())
		{
			...
			{
				$room_number_arr = $this->getRoomNumbers($_GET['room_id'], $_GET['date_from'], $_GET['date_to']);
