	...
	public function pjActionUpdate()
	{
		...
		if ($this->isAdmin())
		{
			if (isset($_POST['room_update']))
			{
				...
				pjUtil::redirect(PJ_INSTALL_URL . "bookAdmin.php?controller=pjAdminRooms&action=pjActionIndex&err=AR01");
				
			} else {
				$arr = pjRoomModel::factory()->find($_GET['id'])->getData();
				if (count($arr) === 0)
				{
					pjUtil::redirect(PJ_INSTALL_URL. "bookAdmin.php?controller=pjAdminRooms&action=pjActionIndex&err=AR08");
				}
