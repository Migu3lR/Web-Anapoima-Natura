...
<fieldset class="fieldset white">
	...
	<form id="frmUpdateShowID" action="<?php echo PJ_INSTALL_URL; ?>bookAdmin.php?controller=pjLocale&amp;action=pjActionIndex" method="post" class="form pj-form b5">
...
<br/>
<form action="<?php echo PJ_INSTALL_URL; ?>bookAdmin.php" method="get" class="float_left pj-form frm-filter b5">
...
<form action="<?php echo PJ_INSTALL_URL; ?>bookAdmin.php?controller=pjLocale&amp;action=pjActionSaveFields" method="post" class="pj-form">
			...
			<table class="pj-table plugin_locale_table" cellpadding="0" cellspacing="0" style="width: <?php echo $table_width;?>px;">
				...
				foreach ($tpl['arr'] as $k => $field_arr)
				{
					...
					<tr class="pj-table-row-<?php echo $k % 2 === 0 ? 'odd' : 'even'; ?>">
						...
						{ 
							?>	
							<td class="dHead"><input type="text" name="i18n[<?php echo $locale_id; ?>][<?php echo $locale_arr['foreign_id']; ?>][title]" value="<?php echo htmlspecialchars(stripslashes(@$locale_arr['content'])); ?>" class="pj-form-field w250" data-key="<?php echo $field_arr['key']; ?>" /></td>
							<?php
						} 
						?>
