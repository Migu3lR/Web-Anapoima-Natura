...
(function ($, undefined) {
	$(function () {
		...
		function formatView (str, obj) {
			return ['<a href="bookAdmin.php?controller=pjInstaller&action=pjActionSecureView&p=', str, '" class="install-view" target="_blank"></a>',
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				buttons: [],
				columns: columns,
				dataUrl: "bookAdmin.php?controller=pjInstaller&action=pjActionSecureGetUpdate",
				dataType: "json",
				fields: fields,
				paginator: false,
				saveUrl: null,
		...
		if ($dialogExecuteAll.length > 0 && dialog) {
			$dialogExecuteAll.dialog({
				...
				buttons: {
					"Execute": function () {
						$.post("bookAdmin.php?controller=pjInstaller&action=pjActionSecureSetUpdate", $grid.find('input[name="record[]"], input[name="module[]"]').serialize()).done(function (data) {
							if (data.status == "OK") {
								...
								$grid.datagrid("load", "bookAdmin.php?controller=pjInstaller&action=pjActionSecureGetUpdate", "", "", content.page, content.rowCount);
		...
		if ($dialogExecute.length > 0 && dialog) {
			$dialogExecute.dialog({
				...
				buttons: {
					"Execute": function () {
						$.post("bookAdmin.php?controller=pjInstaller&action=pjActionSecureSetUpdate", {
