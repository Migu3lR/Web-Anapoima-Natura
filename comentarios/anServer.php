...
function nuevoPost($nombre, $email, $mensaje, $rate, $token){
$sql = connectDB();
...
if (!is_null($rslt)){
	if ($hoy <= $rslt[1]){
		...
		VALUES (NULL, NULL, '$nombre','$email','$mensaje',$rate,'".$rslt[0]."')";
		if (!mysqli_query($sql, $cmd)) $er = bad_insert_request;
		else{
			$cmd = "UPDATE tokens SET estado = 'I' where tokenid = $rslt[0]";
			if (!mysqli_query($sql, $cmd)) $er = token_invalid;
		}
	} else $er = token_invalid;		
} else $er = token_invalid;
