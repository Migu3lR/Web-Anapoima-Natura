		...
		<div class="dashboard-column">
			<div class="dashboard-column-inner">
				<div class="dashboard-block dashboard-outline">
					...
					<div class="dashboard-row"><label><?php __('dash_available_rooms_today'); ?>:</label><span><?php echo (int) @$tpl['arr']['total_rooms'] - (int) @$tpl['arr']['booked_today'] - (int) @$tpl['arr']['pending_today'];?></span></div>
				</div>
				<div class="dashboard-block dashboard-outline dashboard-block-even">
					...
					if (isset($tpl['avail_room_arr']))
					{
						...
						{
							?><div class="dashboard-row"><label><?php echo $v['name'];?></label><span><?php echo $v['cnt']- $v['booked_rooms'] - $v['pending_rooms'];?></span></div><?php
						}
					} 
					?>
				...
				if (isset($tpl['arrivals_arr']) && !empty($tpl['arrivals_arr']))
				{
					foreach ($tpl['arrivals_arr'] as $k => $v)
					{
						...
						<div class="dashboard-block dashboard-outline<?php echo $k % 2 == 0 ? ' dashboard-block-odd' : ' dashboard-block-even'?>">
							...
							if(!empty($v['c_phone']))
							{ 
								?><div class="dashboard-booking-row"><?php echo $v['c_phone'];?></div><?php
							} 
							?>
