...
if (isset($tpl['status']))
{
	...
	if (pjObject::getPlugin('pjInvoice') !== NULL)
	{
		...
		$balanceSubtotal = $tpl['arr']['total'] - (
			...
			($tpl['arr']['total'] * $tpl['option_arr']['o_deposit']) / 100 :
		...
		$balanceAmountDue = $tpl['arr']['total'] + $tpl['arr']['security'] + $tpl['arr']['tax'] - $balanceTotal - $balancePaidDeposit;
		?>
		<form action="<?php echo PJ_INSTALL_URL; ?>bookAdmin.php" method="get" target="_blank" style="display: inline" id="frmBalancePayment">
			...
			<input type="hidden" name="items[0][name]" value="Booking deposit" />
		...
		<form action="<?php echo PJ_INSTALL_URL; ?>bookAdmin.php" method="get" target="_blank" style="display: inline" id="frmAddInvoice">
			<input type="hidden" name="controller" value="pjInvoice" />
			<input type="hidden" name="action" value="pjActionCreateInvoice" />
			<input type="hidden" name="tmp" value="<?php echo md5(uniqid(rand(), true)); ?>" />
			<input type="hidden" name="uuid" value="<?php echo pjUtil::uuid(); ?>" />
