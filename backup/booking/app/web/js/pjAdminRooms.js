	...
	$(function () {
		...
		if ($gallery.length > 0 && gallery) {
			$gallery.gallery({
				...
				resizeUrl: "index.php?controller=pjGallery&action=pjActionResizeGallery&id={:id}&foreign_id=" + myGallery.foreign_id + "&hash=" + myGallery.hash + ($frmUpdateRoom.length > 0 ? "&query_string=" + encodeURIComponent("controller=pjAdminRooms&action=pjActionUpdate&id=" + myGallery.foreign_id + "&tab=4") : ""),
