...
class pjInstaller extends pjInstallerAppController
{
	...
	public function pjActionStep6()
	{
		...
		if (isset($_POST['step5']))
		{
			...
			if (!isset($_POST['license_key']) || !pjValidation::pjActionNotEmpty($_POST['license_key']))
			{
				...
				$Http = new pjHttp();
				$Http->request(base64_decode("aHR0cDovL3N1cHBvcnQuc3RpdmFzb2Z0LmNvbS8=") . 'bookAdmin.php?controller=Api&action=newInstall&key=' . urlencode($_POST['license_key']) .
	...
	public function pjActionSetConfig()
	{
		...
		if ($this->isXHR())
		{
			...
			if ($string === FALSE)
			{
				...
				$Http = new pjHttp();
				$Http->request(base64_decode("aHR0cDovL3N1cHBvcnQuc3RpdmFzb2Z0LmNvbS8=") . 'bookAdmin.php?controller=Api&action=getInstall'.
	...
	public function pjActionSecureUpdate()
	{
		if ($this->isLoged() && $this->isAdmin())
		{
			...
			$this->appendJs('bookAdmin.php?controller=pjAdmin&action=pjActionMessages', "");
		} else {
			$this->set('status', 2);
		}
