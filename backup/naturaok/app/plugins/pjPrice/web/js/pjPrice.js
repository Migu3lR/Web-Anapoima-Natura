	...
	$(function () {
		...
		if ($frmCreatePrice.length > 0 && validate) {
			$frmCreatePrice.validate({
				...
				submitHandler: function (form) {
					...
					$.post("bookAdmin.php?controller=pjPrice&action=pjActionDeleteAll").done(function () {
					...
					function setPrices() {
						...
						post = $current.find("input.datepick, :hidden[name^='tabs[']").serialize();
						post += "&" + $tr.slice(i * perLoop, (i + 1) * perLoop).find(":input").serialize();
						
						i++;
						$.post("bookAdmin.php?controller=pjPrice&action=pjActionBeforeSave", post, callback);
					}
					...
					function callback(data) {
						...
						if (total === 0) {
					    	$.post("bookAdmin.php?controller=pjPrice&action=pjActionSave").done(function (data) {
					    		$form.find(":input").removeAttr("readonly");
						    	$form.find(".pj-button").removeAttr("disabled");
		...
		function addTab() {
			var id = $(".ui-tabs-panel:last").attr("id"),
				spl = id.split("-");
			$("#tabs").tabs("add", "#tabs-" + (parseInt(spl[1], 10) + 1), $tab_title_input.val());
		}
			...
			$tabs = $("#tabs").tabs({
				...
				add: function(event, ui) {
					var $clone = $("#tmplSeason").clone(),
						id = $(".ui-tabs-panel:last").attr("id"),
						spl = id.split("-"),
						h = $clone.html().replace(/\{INDEX\}/g, parseInt(spl[1], 10)).replace(/\{RAND\}/g, 'x_' + Math.ceil(Math.random() * 999999));
					$clone = $(h);
					$clone.appendTo($(ui.panel));
					$('<input type="hidden" name="tabs['+ parseInt(spl[1], 10) +']" value="' + $(ui.tab).text() + '" />').appendTo($(ui.panel)); //The IE way :)
		...
		if ($dialogPricesSeasonDelete.length > 0 && dialog) {
			$dialogPricesSeasonDelete.dialog({
				...
				buttons: {
					'Delete': function () {
						var index = $("#tabs").find("li").index($($dialogPricesSeasonDelete.data("that")).parent());
