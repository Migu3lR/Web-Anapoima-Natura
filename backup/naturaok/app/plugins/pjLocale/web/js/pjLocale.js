...
(function ($, undefined) {
	$(function () {
		...
		if ($tabs.length > 0 && tabs) {
			$tabs.tabs({
				select: function (event, ui) {
					switch (ui.index) {
						case 0:
							window.location.href = 'bookAdmin.php?controller=pjAdminOptions&action=pjActionIndex&tab=0';
							break;
					}
				}
			});
	    ...
	    if ($dialogShowID.length > 0 && dialog) 
		{
	    	$dialogShowID.dialog({
				modal: true,
				autoOpen: false,
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				buttons: [{type: "delete", url: "bookAdmin.php?controller=pjLocale&action=pjActionDeleteLocale&id={:id}", beforeShow: onBeforeShow}],
				...
				dataUrl: "bookAdmin.php?controller=pjLocale&action=pjActionGetLocale",
				dataType: "json",
				fields: ['language_iso', 'file', 'is_default', 'sort'],
				paginator: false,
				saveUrl: "bookAdmin.php?controller=pjLocale&action=pjActionSaveLocale&id={:id}",
				sortable: true,
				sortableUrl: "bookAdmin.php?controller=pjLocale&action=pjActionSortLocale"
			});
			...
			$(document).on("click", ".btn-add", function (e) {
				...
				$.post("bookAdmin.php?controller=pjLocale&action=pjActionSaveLocale").done(function (data) {
					...
					$grid.datagrid("load", "bookAdmin.php?controller=pjLocale&action=pjActionGetLocale");
				});
				return false;
				...
				$.post("bookAdmin.php?controller=pjLocale&action=pjActionSaveDefault", {
