...
class pjFront extends pjAppController
{
	...
	public function __construct()
	{
		...
		$this->set('isTrialExpired', $_SESSION[$this->defaultStatus]);
	}
	...
	public function pjActionCancel()
	{
		...
		if (isset($_POST['booking_cancel']))
		{
			...
			$this->appendCss('bookAdmin.php?controller=pjFront&action=pjActionLoadCss&cid='.$_GET['cid'].'&layout=1', PJ_INSTALL_URL, true);
		}
	}
	...
	public function pjActionLoadCss()
	{
		...
		$arr[] = array('file' => 'bootstrap.min.css', 'path' => PJ_LIBS_PATH . 'pjQ/bootstrap/css/');
		$arr[] = array('file' => 'font-awesome.min.css', 'path' => PJ_THIRD_PARTY_PATH . 'font-awesome/css/');
		$arr[] = array('file' => 'jquery-ui.min.css', 'path' => PJ_LIBS_PATH . 'pjQ/css/');
		$arr[] = array('file' => 'timepicker.css', 'path' => PJ_LIBS_PATH . 'pjQ/bootstrap/css/');
		...
		foreach ($arr as $item)
		{
			...
			if ($string !== FALSE)
			{
				echo str_replace(
					...
					array(
						PJ_LIBS_PATH . 'pjQ/bootstrap/fonts/glyphicons-',
			...
			if (isset($_GET['room_id']) && (int) $_GET['room_id'] > 0)
			{
				...
				if (!isset($_GET['cnt']) || (int) $_GET['cnt'] === 0)
				{
					$arr = $pjRoomModel
						->select(sprintf("t1.*, t2.content AS `name`, t3.content AS `description`,
							...
							(SELECT MIN(LEAST(`mon`, `tue`, `wed`, `thu`, `fri`, `sat`, `sun`)) FROM `%2\$s` WHERE `foreign_id` = `t1`.`id` LIMIT 1) AS `price_from`
							",
					...
					$arr = $pjRoomModel
						->select(sprintf("t1.*, t2.content AS `name`, t3.content AS `description`,
							(SELECT MIN(LEAST(`mon`, `tue`, `wed`, `thu`, `fri`, `sat`, `sun`)) FROM `%1\$s` WHERE `foreign_id` = `t1`.`id` LIMIT 1) AS `price_from`
							",
							pjPriceModel::factory()->getTable()
						))
						->join('pjMultiLang', "t2.model='pjRoom' AND t2.foreign_id=t1.id AND t2.field='name' AND t2.locale='".$this->getLocaleId()."'", 'left outer')
				...
				if (!empty($arr))
				{
					...
					$arr['real_price_from'] = $this->getRoomPrice(
							...
							$_SESSION[$this->defaultStore]['date_to']['adults'],
							$_SESSION[$this->defaultStore]['date_to']['children']
					);
				}
				# --
			...
			switch ($_GET['payment_method'])
			{
				case 'paypal':
					$this->set('params', array(
						...
						'notify_url' => PJ_INSTALL_URL . 'bookAdmin.php?controller=pjFront&action=pjActionConfirmPaypal&cid=' . $_GET['cid'],
				...
				case 'authorize':
					$this->set('params', array(
						...
						'x_relay_url' => PJ_INSTALL_URL . 'bookAdmin.php?controller=pjFront&action=pjActionConfirmAuthorize&cid=' . $_GET['cid'],
	...
	public function pjActionProcessOrder()
	{
		...
		if ($this->isXHR())
		{
			...
			if ((int) $this->option_arr['o_bf_captcha'] === 3 && (!isset($STORE['form']) || !isset($STORE['form']['captcha']) || @$_SESSION[$this->defaultCaptcha] != strtoupper($STORE['form']['captcha'])))
			{
			...
			{
				$data['c_arrival'] = ($FORM['meridian'] == 'AM' 
					? $FORM['hour']
					: (int) $FORM['hour'] + 12) . ":" . $FORM['minute'];
			}
			...
			foreach ($STORE['content'] as $room_id => $rooms_arr)
			{
				...
				{
					$pjBookingRoomModel->addBatchRow(array($booking_id, (int) $room_id, @$avail_room_number_arr[$room_id]['room_number_ids'][$i], (int) $room['adults'], (int) $room['children'], (float) $room['raw_price']));
					$i += 1;
				}
			}
	...
	{
		$STORE =& $_SESSION[$this->defaultStore];
		if (is_null($STORE) || empty($STORE) || !isset($STORE['date_from'], $STORE['date_to'], $STORE['adults'], $STORE['children'], $STORE['all_rooms']))
		{
			return FALSE;
		}
		$nightMode = $this->option_arr['o_price_based_on'] == "nights";
		...
		$arr = pjRoomModel::factory()
			...
			->where('t1.adults >=', $STORE['adults'])
			->where('t1.children >=', $STORE['children'])
			->findAll()
			->getDataPair('id');
	...
	static protected function setLastUsage($opts, $foreign_id)
	{
		if (is_array($opts)
			...
			&& stripos($_SERVER['HTTP_REFERER'], 'stivaweb.com') === FALSE
