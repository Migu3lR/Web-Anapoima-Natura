	...
	public function pjActionUpdate()
	{
		...
		if ($this->isAdmin())
		{
			if (isset($_POST['extra_update']))
			{
				...
				pjUtil::redirect(PJ_INSTALL_URL . "bookAdmin.php?controller=pjAdminExtras&action=pjActionIndex&err=AE01");
				
			} else {
				$arr = pjExtraModel::factory()->find($_GET['id'])->getData();
				if (count($arr) === 0)
				{
					pjUtil::redirect(PJ_INSTALL_URL. "bookAdmin.php?controller=pjAdminExtras&action=pjActionIndex&err=AE08");
				}
