...
class pjAdmin extends pjAppController
{
	...
	protected $defaultApi = 'SFW_Api';
	
	protected $defaultApiSecret = 's8_@!kOd=1{^';
	...
	public function beforeFilter()
	{
		...
		$this->set('api_arr', @$_SESSION[$this->defaultApi]);
	}
	...
	public function pjActionIndex()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			if (!isset($_GET['preview']))
			{
				...
				$isDashboardReady = !in_array(0, array(
					$this->option_arr['o_welcome_done_1'], 
					$this->option_arr['o_welcome_done_2'], 
					$this->option_arr['o_welcome_done_3'], 
					$this->option_arr['o_welcome_done_4'], 
					$this->option_arr['o_welcome_done_5'], 
					$this->option_arr['o_welcome_done_6']
				));
				if (!$isDashboardReady)
				{
					//pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdmin&action=pjActionWelcome");
				}
			}
	...
	public function pjActionWelcomeGet()
	{
		$this->setAjax(true);
	...
	public function pjActionWelcomeSet()
	{
		$this->setAjax(true);
		...
		if ($this->isXHR() && $this->isLoged())
		{
			if (isset($_POST['index']))
			{
				pjOptionModel::factory()
					...
					->where('`key`', 'o_welcome_done_' . (int) $_POST['index'])
					->limit(1)
					->modifyAll(array(
						'value' => '1|0::1'
					));
			...
			if (isset($_POST['switch']))
			{
				pjOptionModel::factory()
					->where('foreign_id', $this->getForeignId())
					->where("`key` LIKE 'o_welcome_done_%'")
					->modifyAll(array(
						'value' => '1|0::1'
					));
				
				pjAppController::jsonResponse(array('status' => 'OK', 'code' => 200, 'text' => 'Item(s) has been updated.'));
			}
	...
	public function pjActionLogin()
	{
		...
		if (is_file($login_api))
		{
			...
			if (is_object($apiLogin))
			{
				...
				$unreadTickets = $apiLogin->unreadTickets()->getResult();
				$_SESSION[$this->defaultApi] = array(
					'checkStatus' => $apiData,
		...
		{
			# API <--
			if (isset($apiData, $apiData['result'], $apiData['result']['is_forbidden']) && $apiData['result']['is_forbidden'])
			{
	...
	public function pjActionSupport()
	{
		$uuid = basename(dirname($_SERVER['PHP_SELF']));
		$api_arr = @$_SESSION[$this->defaultApi];
		if ($api_arr !== FALSE && isset($api_arr['unreadTickets']))
		{
			$api_arr['unreadTickets']['result']['cnt'] = 0;
			$_SESSION[$this->defaultApi]['unreadTickets'] = $api_arr['unreadTickets'];
			$this->set('api_arr', $api_arr);
		}
	}
	
	public function pjActionUpgrade()
	{
		$uuid = basename(dirname($_SERVER['PHP_SELF']));
	}
	...
	{
		$this->setLayout('pjActionApiIframe');
	
		$uuid = basename(dirname($_SERVER['PHP_SELF']));
		$hash = sha1($uuid . $this->defaultApiSecret);
