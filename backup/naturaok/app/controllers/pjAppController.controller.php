...
class pjAppController extends pjController
{
	...
	public function pjActionAfterInstall()
	{
		pjInvoiceConfigModel::factory()->set('id', 1)->modify(array(
			'o_booking_url' => "bookAdmin.php?controller=pjAdminBookings&action=pjActionUpdate&uuid={ORDER_ID}"
		));
	...
	{
		$id = pjCalendarModel::factory()->setAttributes(array('user_id' => 1))->insert()->getInsertId();
	...
	public static function getTokens($booking_arr, $option_arr)
    {
		...
		$cancelURL = PJ_INSTALL_URL . 'bookAdmin.php?controller=pjFront&action=pjActionCancel&cid='.$booking_arr['calendar_id'].'&id='.$booking_arr['id'].'&hash='.sha1($booking_arr['id'].$booking_arr['created'].PJ_SALT);
		...
		$replace = array(
			@$pt[$booking_arr['c_title']], $booking_arr['c_fname'], $booking_arr['c_lname'], $booking_arr['c_phone'], $booking_arr['c_email'],
			$booking_arr['c_arrival'], $booking_arr['c_notes'], $booking_arr['c_company'], $booking_arr['c_address_1'], $booking_arr['c_city'],
