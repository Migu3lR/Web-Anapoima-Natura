	...
	$(function () {
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "bookAdmin.php?controller=pjAdminRestrictions&action=pjActionUpdateRestriction&id={:id}"},
				          {type: "delete", url: "bookAdmin.php?controller=pjAdminRestrictions&action=pjActionDeleteRestriction&id={:id}"}],
				...
				dataUrl: "bookAdmin.php?controller=pjAdminRestrictions&action=pjActionGetRestrictions",
				dataType: "json",
				fields: ['date_from', 'date_to', 'id', 'restriction_type'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "bookAdmin.php?controller=pjAdminRestrictions&action=pjActionDeleteRestrictionBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "bookAdmin.php?controller=pjAdminRestrictions&action=pjActionSaveRestriction&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		if ($dialogAddRestriction.length > 0 && dialog) {
			...
			$dialogAddRestriction.dialog({
				...
				open: function () {
					...
					$.get("bookAdmin.php?controller=pjAdminRestrictions&action=pjActionAddRestriction").done(function (data) {
						...
						$dialogAddRestriction.find("form").validate({
							...
							submitHandler: function (form) {
								$.post("bookAdmin.php?controller=pjAdminRestrictions&action=pjActionAddRestriction", $dialogAddRestriction.find("form").serialize()).done(function (data) {
									if(data.code == 200)
									{
										$(".empty-page").remove();
										$("#grid").parent().show();
										var content = $grid.datagrid("option", "content");
										$grid.datagrid("load", "bookAdmin.php?controller=pjAdminRestrictions&action=pjActionGetRestrictions", "date_from", "ASC", content.page, content.rowCount);
									}else{
		...
		if ($dialogUpdateRestriction.length > 0 && dialog) {
			$dialogUpdateRestriction.dialog({
				...
				open: function () {
					...
					$.get("bookAdmin.php?controller=pjAdminRestrictions&action=pjActionUpdateRestriction", {
						...
						$dialogUpdateRestriction.find("form").validate({
							...
							wrapper: "em",
							submitHandler: function (form) {
								$.post("bookAdmin.php?controller=pjAdminRestrictions&action=pjActionUpdateRestriction", $dialogUpdateRestriction.find("form").serialize()).done(function (data) {
									if(data.code == 200)
									{
										$(".empty-page").remove();
										$("#grid").parent().show();
