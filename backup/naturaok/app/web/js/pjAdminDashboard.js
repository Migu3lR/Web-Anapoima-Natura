function drawCurveTypes(jsonData1, jsonData2) {
	...
	var handler = function(e, data) {
		...
		if (m !== null) {
			window.location.href = 'bookAdmin.php?controller=pjAdminBookings&action=pjActionCalendar&format=Y-m-d&selected_date=' + data.getValue(parseInt(m[1], 10), 0);
		}
	};
	...
	jQuery("#chart-2").appendTo("#charts-2");
}
...
(function ($, undefined) {
	$(function () {
		...
		{
			$.when($.get('bookAdmin.php?controller=pjAdmin&action=pjActionChartGet&type=1'), $.get('bookAdmin.php?controller=pjAdmin&action=pjActionChartGet&type=2')).done(function (a1, a2) {
