			...
			$frmUpdateProfile.validate({
				rules: {
					"email": {
						required: true,
						email: true,
						remote: "bookAdmin.php?controller=pjAdmin&action=pjActionProfileEmail"
					},
					"password": "required",
					"name": "required"
				},
			...
			$dialogWelcomeDone.dialog({
				...
				buttons: (function (){
					...
					btns[myLabel.btn_done] = function () {
						$.post("bookAdmin.php?controller=pjAdmin&action=pjActionWelcomeSet", {
							index: $dialogWelcomeDone.data("index")
						}).done(function (data) {
							if (data.status === "OK") {
								$.get("bookAdmin.php?controller=pjAdmin&action=pjActionWelcomeGet").done(function (data) {
			...
			$dialogWelcomeSwitch.dialog({
				...
				buttons: (function (){
					...
					btns[myLabel.btn_preview] = function () {
						window.location.href = "bookAdmin.php?controller=pjAdmin&action=pjActionIndex&preview=1";
					};
					btns[myLabel.btn_switch] = function () {
						$.post("bookAdmin.php?controller=pjAdmin&action=pjActionWelcomeSet", {
							...
							if (data.status === "OK") {
								window.location.href = "bookAdmin.php?controller=pjAdmin&action=pjActionIndex";
							}
						});
