	...
	$(function () {
		...
		if ($("#grid_packages").length > 0 && datagrid) {
			...
			var $grid_packages = $("#grid_packages").datagrid({
				buttons: [{type: "delete", url: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionDeletePackage&id={:id}"}],
				columns: [
				...
				dataUrl: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionGetPackages",
				dataType: "json",
				fields: ['room_id', 'date_from', 'date_to', 'start_day', 'end_day', 'total_price', 'id'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionDeletePackageBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionSavePackage&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		if ($("#grid_frees").length > 0 && datagrid) {
			...
			var $grid_frees = $("#grid_frees").datagrid({
				buttons: [{type: "delete", url: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionDeleteFree&id={:id}"}],
				columns: [
				          {text: myLabel.room, type: "select", sortable: true, editable: true, options: myLabel.rooms, width: 160, editableWidth: 140},
				          {text: myLabel.date_from, type: "date", sortable: true, editable: false, width: 100, editableWidth: 80, 
								jqDateFormat: pjGrid.jqDateFormat, 
				...
				dataUrl: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionGetFrees",
				dataType: "json",
				fields: ['room_id', 'date_from', 'date_to', 'min_length', 'max_length', 'free_nights'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionDeleteFreeBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionSaveFree&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		if ($("#grid_codes").length > 0 && datagrid) {
			...
			var $grid_codes = $("#grid_codes").datagrid({
				buttons: [{type: "delete", url: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionDeleteCode&id={:id}"}],
				columns: [
				...
				dataUrl: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionGetCodes",
				dataType: "json",
				fields: ['room_id', 'date_from', 'date_to', 'promo_code', 'type', 'discount'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionDeleteCodeBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionSaveCode&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		if ($dialogAddPackage.length > 0 && dialog) {
			$dialogAddPackage.dialog({
				...
				open: function () {
					...
					$.get("bookAdmin.php?controller=pjAdminDiscounts&action=pjActionAddPackage").done(function (data) {
						...
						$dialogAddPackage.find("form").validate({
							...
							submitHandler: function (form) {
								...
								$.post("bookAdmin.php?controller=pjAdminDiscounts&action=pjActionAddPackage", $dialogAddPackage.find("form").serialize()).done(function (data) {
									$(".empty-page").remove();
									$("#grid_packages").parent().show();
									var content = $grid_packages.datagrid("option", "content");
									$grid_packages.datagrid("load", "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionGetPackages", "date_from", "ASC", content.page, content.rowCount);
								}).always(function () {
		...
		if ($dialogAddFree.length > 0 && dialog) {
			$dialogAddFree.dialog({
				...
				open: function () {
					$dialogAddFree.html("");
					$.get("bookAdmin.php?controller=pjAdminDiscounts&action=pjActionAddFree").done(function (data) {
						...
						$dialogAddFree.find("form").validate({
							...
							submitHandler: function (form) {
								...
								$.post("bookAdmin.php?controller=pjAdminDiscounts&action=pjActionAddFree", $dialogAddFree.find("form").serialize()).done(function (data) {
									$(".empty-page").remove();
									$("#grid_frees").parent().show();
									var content = $grid_frees.datagrid("option", "content");
									$grid_frees.datagrid("load", "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionGetFrees", "date_from", "ASC", content.page, content.rowCount);
								}).always(function () {
		...
		if ($dialogAddCode.length > 0 && dialog) {
			$dialogAddCode.dialog({
				...
				open: function () {
					$dialogAddCode.html("");
					$.get("bookAdmin.php?controller=pjAdminDiscounts&action=pjActionAddCode").done(function (data) {
						...
						$dialogAddCode.find("form").validate({
							...
							submitHandler: function (form) {
								...
								$.post("bookAdmin.php?controller=pjAdminDiscounts&action=pjActionAddCode", $dialogAddCode.find("form").serialize()).done(function (data) {
									$(".empty-page").remove();
									$("#grid_codes").parent().show();
									var content = $grid_codes.datagrid("option", "content");
									$grid_codes.datagrid("load", "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionGetCodes", "date_from", "ASC", content.page, content.rowCount);
								}).always(function () {
		...
		if ($dialogPackageItems.length > 0 && dialog) {
			...
			$dialogPackageItems.dialog({
				...
				open: function () {
					...
					$.get("bookAdmin.php?controller=pjAdminDiscounts&action=pjActionPackageItems").done(function (data) {
						$dialogPackageItems.html(data);
						
						$grid_items = $("#grid_items").datagrid({
							buttons: [{type: "delete", url: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionDeletePackageItem&id={:id}"}],
							columns: [
							...
							dataUrl: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionGetPackageItems&package_id=" + $dialogPackageItems.data("package_id"),
							dataType: "json",
							fields: ['adults', 'children', 'price'],
							paginator: false,
							saveUrl: "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionSavePackageItem&id={:id}",
							select: {
								field: "id",
								name: "record[]"
							},
		...
		if ($dialogPackageAddItem.length > 0 && dialog) {
			$dialogPackageAddItem.dialog({
				...
				open: function () {
					...
					$.get("bookAdmin.php?controller=pjAdminDiscounts&action=pjActionPackageAddItem", {
						...
						$dialogPackageAddItem.find("form").validate({
							...
							wrapper: "em",
							submitHandler: function (form) {
								$.post("bookAdmin.php?controller=pjAdminDiscounts&action=pjActionPackageAddItem", $(form).serialize()).done(function (data) {
									$dialogPackageAddItem.dialog("close");
									if (data.status == "OK") {
										var content = $grid_items.datagrid("option", "content");
										$grid_items.datagrid("load", "bookAdmin.php?controller=pjAdminDiscounts&action=pjActionGetPackageItems&package_id=" + $dialogPackageAddItem.data("package_id"), "adults", "ASC", content.page, content.rowCount);
									}
								});
								return false;
							}
