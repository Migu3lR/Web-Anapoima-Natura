					...
					foreach ($column_arr as $index => $col)
					{
						if (!empty($col['content']))
						{
							...
							if (!isset($content['restriction_type']))
							{
								...
								{
									$name_arr[] = stripslashes($content['c_fname']);
								}
								if (!empty($content['c_lname']))
								{
									$name_arr[] = stripslashes($content['c_lname']);
								}
								if (!empty($content['c_email']))
								{
									$tooltip_arr[] = stripslashes($content['c_email']);
								}
								if (!empty($content['c_phone']))
								{
									$tooltip_arr[] = __('booking_c_phone', true) . ': '.stripslashes($content['c_phone']);
								}
								$tooltip_arr[] = __('booking_adults', true) . ': '.stripslashes($content['adults']) . ', ' . __('booking_children', true) . ': '.stripslashes($content['children']);
								$other_arr = array();
							...
							if ($col['colspan'] > 1 || (
									!isset($column_arr[$index + 1]) 
									|| ($column_arr[$index + 1]['colspan'] > 1 && empty($column_arr[$index + 1]['content']))
								)
							)
							{
