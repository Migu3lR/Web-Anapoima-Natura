		...
		if ($('#frmStep6').length > 0) {
			$('#frmStep6').validate({
				...
				submitHandler: function(form) {
					...
					$.post("bookAdmin.php?controller=pjInstaller&action=pjActionSetConfig&install=1").done(function (data) {
						if (data.code == 200) { 
							$ready.eq(0).addClass("i-option-ok").removeClass("i-option-load i-option-ready");
							$ready.eq(1).addClass("i-option-load");
							$.post("bookAdmin.php?controller=pjInstaller&action=pjActionSetDb&install=1").done(function (data) {
								if (data.code == 200) {
