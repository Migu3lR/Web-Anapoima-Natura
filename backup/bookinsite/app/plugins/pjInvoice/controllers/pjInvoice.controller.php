...
class pjInvoice extends pjInvoiceAppController
{
	...
	public function pjActionCreateInvoice()
	{
		...
		if (isset($_POST['invoice_create']))
		{
			...
			{
				pjUtil::redirect(PJ_INSTALL_URL . "bookAdmin.php?controller=pjInvoice&action=pjActionInvoices&err=PIN06");
			}
			...
			pjUtil::redirect(PJ_INSTALL_URL . "bookAdmin.php?controller=pjInvoice&action=pjActionInvoices&err=$err");
		} else {
			...
			$this
				...
				->appendJs('bookAdmin.php?controller=pjAdmin&action=pjActionMessages', PJ_INSTALL_URL, true)
			;
		}
	}
		...
		if (isset($_POST['invoice_post']))
		{
			if (isset($_FILES['y_logo']) && !empty($_FILES['y_logo']['tmp_name']))
			{
				...
				if ($pjImage->load($_FILES['y_logo']))
				{
					...
					pjUtil::redirect(PJ_INSTALL_URL . "bookAdmin.php?controller=pjInvoice&action=pjActionIndex&err=PIN03&errTime=" . $time);
				}
			}
			$data = array();
			...
			pjUtil::redirect(PJ_INSTALL_URL . "bookAdmin.php?controller=pjInvoice&action=pjActionIndex&err=PIN02&tab_id=" . $_POST['tab_id']);
		}
		...
		switch ($_POST['payment_method'])
		{
			case 'paypal':
				$this->set('params', array(
					...
					'return' => PJ_INSTALL_URL . 'bookAdmin.php?controller=pjInvoice&action=pjActionView&uuid=' . $arr['uuid'],
					'notify_url' => PJ_INSTALL_URL . 'bookAdmin.php?controller=pjInvoice&action=pjActionConfirmPaypal&cid=' . $arr['foreign_id']
				));
				break;
			case 'authorize':
				$this->set('params', array(
					...
					'x_receipt_link_url' => PJ_INSTALL_URL . 'bookAdmin.php?controller=pjInvoice&action=pjActionView&uuid=' . $arr['uuid'],
					'x_relay_url' => PJ_INSTALL_URL . 'bookAdmin.php?controller=pjInvoice&action=pjActionConfirmAuthorize&cid=' . $arr['foreign_id']
				));
				break;
		}
	...
	public function pjActionSend()
	{
		...
		if ($this->isXHR() && $this->isLoged() && $this->isInvoiceReady())
		{
			...
			if (isset($_POST['uuid']) && !empty($_POST['uuid']))
			{
				...
				$view_url = PJ_INSTALL_URL . 'bookAdmin.php?controller=pjInvoice&action=pjActionView&id=' . $_POST['id'] . '&uuid=' . $_POST['uuid'];
	...
	public function pjActionUpdate()
	{
		...
		if (isset($_POST['invoice_update']))
		{
			...
			{
				pjUtil::redirect(PJ_INSTALL_URL . "bookAdmin.php?controller=pjInvoice&action=pjActionInvoices&err=PIN04");
			}
			...
			pjUtil::redirect(PJ_INSTALL_URL . "bookAdmin.php?controller=pjInvoice&action=pjActionUpdate&id=".$_POST['id']."&err=PIN05");
		}
