	...
	$(function () {
		...
		if ($("#grid").length > 0 && datagrid) {
			var $grid = $("#grid").datagrid({
				buttons: [{type: "delete", url: "bookAdmin.php?controller=pjOneAdmin&action=pjActionDelete&id={:id}"}],
				...
				dataUrl: "bookAdmin.php?controller=pjOneAdmin&action=pjActionGet",
				dataType: "json",
				fields: ['name', 'url', 'email', 'password'],
				paginator: {
					actions: [
					   {text: "Delete selected", url: "bookAdmin.php?controller=pjOneAdmin&action=pjActionDeleteBulk", render: true, confirmation: "Are you sure you want to delete selected records?"}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "bookAdmin.php?controller=pjOneAdmin&action=pjActionSave&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
			...
			$(document).on("click", ".btn-add", function (e) {
				...
				$.post("bookAdmin.php?controller=pjOneAdmin&action=pjActionSave").done(function (data) {
					...
					$grid.datagrid("load", "bookAdmin.php?controller=pjOneAdmin&action=pjActionGet");
				});
				return false;
			});
		}
