...
{	
	...
	public function pjActionRooms()
	{
		if ($this->isXHR() || isset($_GET['_escaped_fragment_']))
		{
			if (isset($_GET['date_from'], $_GET['date_to'], $_GET['adults'], $_GET['children']))
			{
				...
				if (isset($_SESSION[$this->defaultStore]) &&
					...
					isset($_SESSION[$this->defaultStore]['adults']) &&
					isset($_SESSION[$this->defaultStore]['children']) &&
					(
						$_SESSION[$this->defaultStore]['date_from'] != $date_from ||
						$_SESSION[$this->defaultStore]['date_to'] != $date_to ||
						$_SESSION[$this->defaultStore]['adults'] != $adults ||
						$_SESSION[$this->defaultStore]['children'] != $children
					))
				{
				...
				$arr = $pjRoomModel
					...
					->where('t1.id > 0')
					->where('t1.calendar_id', $_GET['cid'])
					->where('t1.adults * t1.cnt >=', $_GET['adults'])
					->where('t1.children * t1.cnt >=', $_GET['children'])
					->findAll()
					->toArray('gallery', '~:~')
					->toArray('medium', '~:~')
					->toArray('large', '~:~')
