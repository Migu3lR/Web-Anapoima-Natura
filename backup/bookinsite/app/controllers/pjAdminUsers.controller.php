...
class pjAdminUsers extends pjAdmin
{
	...
	public function pjActionCreate()
	{
		...
		if ($this->isAdmin())
		{
			if (isset($_POST['user_create']))
			{
				...
				{
					pjUtil::redirect(PJ_INSTALL_URL . "bookAdmin.php?controller=pjAdminUsers&action=pjActionIndex&err=AU09");
				}
	...
	public function pjActionDeleteUser()
	{
		...
		if ($this->isXHR() && $this->isLoged() && $this->isAdmin())
		{
			if (isset($_GET['id']) && (int) $_GET['id'] > 0 && $_GET['id'] != $this->getUserId())
			{
			...
			pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 101, 'text' => 'Missing or invalid parameters.'));
		}
		exit;
	}
	...
	public function pjActionIndex()
	{
		...
		if ($this->isAdmin())
		{
			$this->set('is_empty', 0 == pjUserModel::factory()->findCount()->getData());
			
			$this->appendJs('jquery.datagrid.js', PJ_FRAMEWORK_LIBS_PATH . 'pj/js/');
			$this->appendJs('pjAdminUsers.js');
			$this->appendJs('bookAdmin.php?controller=pjAdmin&action=pjActionMessages', PJ_INSTALL_URL, true);
		} else {
			$this->set('status', 2);
		}
	}
	...
	public function pjActionSaveUser()
	{
		...
		if ($this->isXHR() && $this->isLoged() && $this->isAdmin())
		{
			if (isset($_POST['column']) && isset($_POST['value']) && !empty($_POST['column']))
			{
				...
				{
					if ($_POST['column'] == 'email' && 0 != $pjUserModel->where('t1.email', $_POST['value'])->where('t1.id!=', $_GET['id'])->findCount()->getData())
					{
						pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 100, 'text' => 'Duplicate email.'));
					}
					if (in_array($_POST['column'], array('email', 'name')) && empty($_POST['value']))
					{
						pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 100, 'text' => 'Empty value.'));
					}
					if ($this->getUserId() == $_GET['id'])
					{
						$isForbidden = FALSE;
						if ($_POST['column'] == 'status' && $_POST['value'] == 'F')
						{
							$isForbidden = TRUE;
						}	
						...
						if ($isForbidden)
						{
							pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 100, 'text' => 'Forbidden.'));
						}
					}
					$pjUserModel->reset()->set('id', $_GET['id'])->modify(array($_POST['column'] => $_POST['value']));
				} else {
			...
			pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 100, 'text' => 'Missing or invalid data.'));
		}
		exit;
	}
	...
	public function pjActionStatusUser()
	{
		...
		if ($this->isXHR() && $this->isLoged() && $this->isAdmin())
		{
			if (isset($_POST['record']) && !empty($_POST['record']))
			{
				...
				pjAppController::jsonResponse(array('status' => 'OK', 'code' => 200, 'text' => 'User status has been updated.'));
			}
			pjAppController::jsonResponse(array('status' => 'ERR', 'code' => 100, 'text' => 'Missing or invalid data.'));
		}
		exit;
	...
	public function pjActionUpdate()
	{
		...
		if ($this->isAdmin())
		{
			...
			{
				$required = array('id', 'role_id', 'email', 'password', 'name', 'status');
				foreach ($required as $index)
				{
				...
				$pjUserModel->reset()->set('id', $_POST['id'])->modify($_POST);
				if (isset($_POST['i18n']))
				{
					pjMultiLangModel::factory()->updateMultiLang($_POST['i18n'], $_POST['id'], 'pjUser');
				}
				pjUtil::redirect(PJ_INSTALL_URL . "bookAdmin.php?controller=pjAdminUsers&action=pjActionIndex&err=AU01");
				
			} else {
				if (!isset($_GET['id']) || empty($_GET['id']))
				{
					pjUtil::redirect(PJ_INSTALL_URL. "bookAdmin.php?controller=pjAdminUsers&action=pjActionIndex&err=AU08");
				}
